...
# CLAUDE.md itself and other root-level named config files
for extra in CLAUDE.md tasks.py requirements.txt setup.py; do
  [[ -f "$ROOT/$extra" ]] && config_files+=("$extra")
done
...
while IFS= read -r line; do
  ...
  if $section_found; then
    if [[ "$line" =~ ^\|[[:space:]]*\`([^\`]+)\`[[:space:]]*\|[[:space:]]*(.+)[[:space:]]*\| ]]; then
      ...
      desc="${BASH_REMATCH[2]}"
      [[ "$file" == "File" ]] && continue
      descriptions["$file"]="${desc%"${desc##*[![:space:]]}"}"
    fi
  fi
done < "$CLAUDE_MD"
